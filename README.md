...
- **남의 환경에서 처음부터 세워본 적이 없다.** `markdown` 백엔드로 빈 레포에서 왕복하는 것까지는
  확인했지만, 실제로 설치해서 한 사이클을 돌린 건 만든 사람 환경뿐이다.
- **`markdown` 백엔드로 파이프라인 전체를 완주한 적이 없다.** 어댑터 왕복은 검증했으나
  `impl-pipeline` → `review-gate` → `change-walkthrough` 를 markdown 원장으로 끝까지 돌려보지 않았다.
- **한국어가 아닌 프로젝트에서 어떻게 읽히는지 모른다.** 헤딩은 별칭 맵으로 양쪽을 인식하지만
  스킬 본문·게이트 리포트·보드 UI 는 한국어다.
- **검수 시간이 실제로 줄었는지 측정한 적이 없다.** 설계 의도는 "diff 총량이 아니라 판단 필요
  항목 수에 비례" 인데, 전후 비교 데이터가 없다.
